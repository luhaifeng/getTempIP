...
$, win, ready = {
    ...
    Class.pt.config = {
        ...
        loading: { type: 0 },
        tips: { msg: '', follow: '', guide: 0, isGuide: true, style: ['background-color:#23b090; color:#fff;', '#23b090'] },
	...
	frame = [
		...
		'<div class="xubox_dialog" style="margin-top:5px;"><span class="xubox_msg xubox_text" style="' + (ico ? '' : 'padding-left:15px; color:white') + '">' + dialog.msg + '</span></div>',
	...
	shadeStyle = 'z-index:' + zIndex + '; background-color:' + config.shade[1] + '; opacity:' + config.shade[0] + '; filter:alpha(opacity=' + config.shade[0] * 100 + ');';
        ...
        //config.closeBtn[1] && (closebtn = '<a class="xubox_close xulayer_png32 xubox_close' + config.closeBtn[0] + '" href="javascript:;" style="display:none"></a>');
        config.closeBtn[1] && (closebtn = '<div style="margin-top:0px; margin-bottom:5px;  padding-bottom:20px;"><img src="http://www.fairvo.com/mobile/tc.png" class="xubox_close xulayer_png32 xubox_close' + config.closeBtn[0] + '"  style="padding-top:0px; margin-right:40%; width:28px;  height:28px;  background:none"/></div>');
        config.title[1] && (title = '')

        var btn_panel = "";

        if (config.closeBtn[0] == "0") {
            btn_panel = '<span class="xubox_botton" style="width:100%"></span>';
        } else {
...
+ '" class="' + dom.lay + '">'
    ...
    Class.pt.set = function (times) {
        ...
        switch (config.type) {
            ...
            case 4:
                ...
                break;
            default:
                this.layerMian.css({ 'background-color': '#23b090' });
                if (config.title[1]) {
	...
	layerPage = that.layerPage, layerB = that.layerB, titHeight;
        ...
        (config.area[1] === 'auto') && layerMian.css({ height: titHeight + outHeight + btnHeight +h });
